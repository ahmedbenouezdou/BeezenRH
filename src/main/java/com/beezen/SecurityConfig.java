...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		JWTAuthentificationFilter authFilter = new JWTAuthentificationFilter(super.authenticationManager(),
				customUserDetailsService);
		authFilter.setFilterProcessesUrl("/login");

		http.cors().configurationSource(request -> requestConfiguration).and().csrf().disable().authorizeRequests()
				.and().formLogin().loginPage("/login").permitAll().and().addFilter(authFilter)
